          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
            ...
            {[
              ...
              <div key={label} className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg shrink-0 border border-subtle bg-surface">
                  {icon}
                </div>
                <div>
                  <p className="text-xs font-bold text-brand-400 uppercase tracking-wider mb-0.5">{label}</p>
                  <p className="text-sm" style={{ color: '#c4c0e0' }}>{value}</p>
                </div>
              </div>
            ))}
            ...
            <div className="card-gradient p-5">
              ...
              <p className="text-sm text-muted leading-relaxed">We're taking on new projects. Typical engagement starts within 1–2 weeks.</p>
            </div>
                ...
                {['We review your message within 24h', 'Schedule a 30-min discovery call', 'Receive a tailored proposal'].map((step, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <span className="w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5 text-white"
                      style={{ background: 'linear-gradient(135deg,#4f46e5,#c026d3)' }}>{i + 1}</span>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
            ...
            {status === 'success' ? (
              <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <p className="mt-2 text-sm text-muted max-w-xs mx-auto leading-relaxed">Thanks for reaching out. We'll be in touch within 24 hours.</p>
                <button onClick={() => setStatus('idle')}
                  className="mt-7 text-sm text-brand-400 hover:text-brand-300 transition-colors underline underline-offset-4">
              ...
              <form onSubmit={handleSubmit} noValidate className="card p-8 space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-xs font-bold text-brand-400 mb-2 uppercase tracking-wider">Name</label>
                    <input type="text" placeholder="Your name" value={form.name} onChange={set('name')}
                      className={`input-field ${errors.name ? 'input-error' : ''}`} />
                    {errors.name && <p className="mt-1.5 text-xs text-red-400">{errors.name}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-brand-400 mb-2 uppercase tracking-wider">Email</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-brand-400 mb-2 uppercase tracking-wider">Message</label>
                ...
                <p className="text-center text-xs text-faint">No spam, ever. We'll only use your info to respond to your message.</p>
              </form>
            )}
          </motion.div>
        </div>
