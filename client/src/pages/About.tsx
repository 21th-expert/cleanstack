    ...
    <>
      ...
      <section className="border-y border-subtle py-28" style={{ background: 'var(--bg-3)' }}>
        <div className="section">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {values.map((v, i) => (
              <motion.div key={v.title} custom={i} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-40px' }} variants={fadeUp}
                className="card-gradient p-8 group">
      ...
      <section className="section py-28" style={{ background: 'var(--bg-2)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {team.map((member, i) => (
            <motion.div key={member.name} custom={i} initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp}
